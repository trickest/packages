...
## How it Works
A [Trickest](https://trickest.com) workflow get the initial dataset from [hugovk's Top PyPI packages project](https://hugovk.github.io/top-pypi-packages/) then perform multiple checks to find any red flags that could indicate that a package is (or can be) compromised.

![Trickest Workflow](packages.png "Trickest Workflow - Packages")
### TB; DZ (Too big; didn't zoom)
- The initial dataset is collected from [hugovk's Top PyPi packages project](https://hugovk.github.io/top-pypi-packages/), which contains a list of PyPI's top 5000 most downloaded packages, updated monthly. (Thanks, [@hugovk](https://github.com/hugovk)!)
...
- This node branches off into 3 checks:
    - The package latest release date and repository latest commit date are compared. If a package version has been released _after_ the latest commit date, the package is flagged.
